...
router.post("/register", async (req, res) => {
    try {
        ...
        if (!name || !email || !phone || !work || !password || !cpassword) {
            res.status(422).json({ "error": "plz filled the all the field properly...!" })   // res.json() function used to sent json response
        }
...
router.post("/contact", auth, async (req, res) => {
    try {
        ...
        if (!name || !email || !phone || !message) {
            res.status(422).json({ "error": "plz filled the all the field properly...! " });
        }
        else {
